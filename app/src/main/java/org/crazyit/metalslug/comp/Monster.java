...
public class Monster
{
	...
	public Monster(int type)
	{
		...
		//如果怪物是boos逗比龙的话，那么他的高度就是人的两倍
		else if (type == TYPE_LONG){
			y = Player.Y_DEFALUT*2;
		}
		// 随机计算怪物的X坐标。
...
//                break;
	...
	public void drawAni(Canvas canvas, Bitmap[] bitmapArr)
	{
		...
		if (isDie && dieMaxDrawCount == Integer.MAX_VALUE)
		{
			...
			dieMaxDrawCount = bitmapArr.length;  // ⑤与死亡动画的总帧数相等
		}
		drawIndex = drawIndex % bitmapArr.length;
		// 获取当前绘制的动画帧对应的位图
		Bitmap bitmap = bitmapArr[drawIndex];  // ①根据drawindex来获取当前的帧对应的位图在执行drawAni时候drawIndex只增加一次
...
//            case TYPE_LONG://在这里定义怪物的子弹，都比龙的子弹就是火，到时候要在子弹类里面独立定义一个
//                return Bullet.BULLET_TYPE_1;
			default:
				return 0;
		}
	}
